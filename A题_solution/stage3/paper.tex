...
\noindent \textbf{针对问题五}，进一步扩展为5架无人机至多投放15枚弹干扰3枚导弹的多对多场景。采用任务分配与分组并行优化策略，基于各无人机与导弹的空间关系分配干扰目标。对距视线较远的$FY_4$，采用逆向运动学方法直接求解其到达$M_2$视线的可行性条件，以$9.614$\,s长引信延迟使弹下落至视线高度，成功实现有效遮蔽。5架无人机均实现有效遮蔽，按导弹分组取并集后总遮蔽时长为 \textbf{18.917\,s}。
...
5架无人机均实现了有效遮蔽，总遮蔽时长18.917\,s。$FY_4$通过逆向运动学方法找到了对$M_2$的有效遮蔽方案：以$305.13^\circ$航向（朝$x$负方向、$y$负方向）飞行，利用$9.614$\,s的长引信延迟使烟幕弹下落至$M_2$视线高度（$z \approx 1347$\,m），在$t \approx 14.8$\textasciitilde$18.6$\,s形成遮蔽，与$FY_3$（$t \approx 24.2$\textasciitilde$27.2$\,s）形成时间接力。

\subsubsection{多弹接力分析}

在航向锁定约束下（无人机航向一旦确定不可调整），同一无人机的第2枚弹沿相同方向投放，起爆点位置差异有限，遮蔽窗口高度重叠，难以形成有效时间接力。因此本文每架无人机仅投放1枚弹，依靠多机协同实现时间接力。
