...
% 章节编号直接书写在标题文本中(如"一、问题重述"、"1.1 问题背景")，字体格式见下方titleformat定义
...
\title{\fontsize{22}{28}\selectfont\bfseries 烟幕干扰弹的投放策略研究}
\author{}
\date{}

\titleformat{\section}{\fontsize{14}{18}\selectfont\bfseries\heiti}{}{0em}{}
\titleformat{\subsection}{\fontsize{12}{15}\selectfont\bfseries\heiti}{}{0em}{}
\titleformat{\subsubsection}{\fontsize{12}{15}\selectfont\bfseries\heiti}{}{0em}{}
\begin{document}
\maketitle

% ==================== 摘要 ====================
\begin{center}
{\fontsize{14}{18}\selectfont\bfseries 摘\quad 要}
\end{center}
...
雷达发现导弹时，3枚导弹$M_1$、$M_2$、$M_3$及5架无人机$FY_1$\textasciitilde$FY_5$的位置已知（表\ref{tab:init}）\cite{1}。无人机受领任务后可瞬时调整飞行方向，以70\textasciitilde140\,m/s等高度匀速直线飞行。每架无人机投放两枚弹至少间隔1\,s。
...
本题的核心在于两层耦合：（1）精确描述导弹、无人机、烟幕弹及云团的三维运动轨迹；（2）建立严格的有效遮蔽判定准则。在此基础上，通过优化算法搜索最优投放策略。本文遵循数学建模的一般方法流程\cite{4}。
...
采用粒子群优化（PSO）算法求解\cite{2}。PSO通过群体中粒子的信息共享搜索全局最优，适合目标函数不可解析的非线性优化问题，差分进化（DE）等元启发式算法同样适用于此类问题\cite{3}。算法流程如图\ref{fig:flowchart}所示。
...
最优解中引信延迟$\Delta t = 0$，即投放即起爆。这表明在单弹场景下，让烟幕云团在无人机当前位置（$z=1800$\,m高度）瞬时形成是最优的——弹的平抛下落反而会使云团偏离最佳遮蔽位置。最优航向角$4.20^\circ$接近$x$轴正向，即朝导弹来袭方向飞行，使烟幕弹尽早出现在导弹与目标之间的视线上。灵敏度分析（图\ref{fig:q2_sens}）表明航向角在最优值附近存在明显峰值，遮蔽时长对航向角较为敏感，而对飞行速度相对稳健。
...
\midrule
1 & 0.0000 & 0.0000 & 2.556 & $[4.848,\ 7.404]$ \\
2 & 1.0000 & 0.0000 & 4.257 & $[1.000,\ 5.257]$ \\
3 & 11.993 & 4.017 & 0.000 & 无有效遮蔽 \\
\bottomrule
\end{tabular}
\end{table}
...
三机遮蔽时间窗口基本不重叠（图\ref{fig:q4}）：$FY_1$在早期（$t\approx2$\textasciitilde$6$\,s）遮蔽，$FY_2$在中期（$t\approx15$\textasciitilde$19$\,s）接力，$FY_3$在后期（$t\approx22$\textasciitilde$25$\,s）收尾。$FY_2$最优航向角$304.51^\circ$表明需朝$x$正方向、$y$负方向飞行以接近$M_1$的视线；$FY_3$航向角$94.05^\circ$（接近$y$正方向）则因其初始位置$(6000, -3000, 700)$需朝$y$正方向飞行以接近视线。
...
5架无人机均实现了有效遮蔽（图\ref{fig:q5}），总遮蔽时长18.917\,s。$FY_4$通过逆向运动学方法找到了对$M_2$的有效遮蔽方案：以$305.13^\circ$航向（朝$x$正方向、$y$负方向）飞行，利用$9.614$\,s的长引信延迟使烟幕弹下落至$M_2$视线高度（$z \approx 1347$\,m），在$t \approx 14.8$\textasciitilde$18.6$\,s形成遮蔽，与$FY_3$（$t \approx 24.2$\textasciitilde$27.2$\,s）形成时间接力。
...
\subsection{C. solve\_fix.py -- Q3三弹接力、Q4三机协同、Q5多对多}
\begin{lstlisting}[language=Python, caption={C. solve\_fix.py}]
...
if __name__ == '__main__':
    ...
    # Q5
    log("\n"+"="*50); log("Q5"); log("="*50)
    # 注: 本脚本为中间版本, FY4暂按M3搜索, 得Q5=15.149s;
    # 最终结果18.917s需运行附录D(fy4_inverse.py)+附录E(update_q5.py),
    # 将FY4修正为干扰M2(θ=305.13°, v=134.40, t_rel=4.849, Δt=9.614s)。
...
\subsection{D. fy4\_inverse.py -- FY4逆向运动学搜索}
\begin{lstlisting}[language=Python, caption={D. fy4\_inverse.py}]
import numpy as np
from core import *
from optimizer import calc_time_fast, calc_time_precise
...
\subsection{E. update\_q5.py -- Q5结果整合与result文件输出}
\begin{lstlisting}[language=Python, caption={E. update\_q5.py}]
...
with open("q5_updated.json", 'w') as f:
...
\subsection{F. q5\_multi.py -- Q5多弹接力搜索与result3.xlsx输出}
\begin{lstlisting}[language=Python, caption={F. q5\_multi.py}]
"""
...
with open(r"q5_updated.json") as f:
    R = json.load(f)
